USE [CoSACS]
GO
/****** Object:  StoredProcedure [dbo].[Step1_CreateTempClTables]    Script Date: 8/6/2020 5:08:32 AM ******/
SET ANSI_NULLS ON
GO
...
BEGIN
	...
	--(SELECT custid FROM dbo.customer WHERE custid NOT IN (SELECT custid FROM dbo.cashloan)) t
	
END

GO
/****** Object:  StoredProcedure [dbo].[Step2_DeleteCLAccounts]    Script Date: 8/6/2020 5:08:32 AM ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
...
BEGIN
	...
	--delete from dbo.instalplan_dateaudit where acctno in (select acctno from #tmp) -- nodelete trigger
	...
	--delete from dbo.temp_agr where acctno in (select acctno from #tmp)
	--delete from dbo.COVID_chargesdata_DataFix_Update where acctno in (select acctno from #tmp) -- nodelete trigger
	...
	--delete from dbo.mjkc_acct where acctno in (select acctno from #tmp)
	...
	--delete from dbo.nstk_temp03 where acctno in (select acctno from #tmp)
	...
	--delete from dbo.temp_inst where acctno in (select acctno from #tmp)
	...
	--delete from dbo.nstk_temp02 where acctno in (select acctno from #tmp)
	--delete from dbo.nstk_temp01 where acctno in (select acctno from #tmp)
	delete from dbo.CollectionReason where acctno in (select acctno from #tmp)
	--delete from dbo.instalback where acctno in (select acctno from #tmp)
	--delete from dbo.instalplanbak where acctno in (select acctno from #tmp)
	...
	--delete from dbo.nstk_temp04 where acctno in (select acctno from #tmp)
	delete from dbo.DataCleanLog where acctno in (select acctno from #tmp)
	--delete from dbo.as400trans1 where acctno in (select acctno from #tmp)
	--delete from dbo.instalplan1 where acctno in (select acctno from #tmp)
	--delete from dbo.as400fin1 where acctno in (select acctno from #tmp)
	...
	--delete from dbo.TEMP_6629587_CMStrategyAcct where acctno in (select acctno from #tmp)
	...
	--delete from dbo.COVID_Instalplan_DataFix_Update_Backup where acctno in (select acctno from #tmp) -- nodelete trigger
	--delete from dbo.COVID_Instalplan_DataFix_Update where acctno in (select acctno from #tmp) -- nodelete trigger
	delete from dbo.unposteddelsDaily where acctno in (select acctno from #tmp)
	--delete from dbo.followbad where acctno in (select acctno from #tmp)
	...
	--delete from dbo.finintdels where acctno in (select acctno from #tmp)
	...
	--delete from dbo.del_temp where acctno in (select acctno from #tmp)
	--delete from dbo.wdelivery_net where acctno in (select acctno from #tmp)
	delete from dbo.BailliffUnsuccessful where acctno in (select acctno from #tmp)
	--delete from dbo.act_temp where acctno in (select acctno from #tmp)
	--delete from dbo.fin_temp where acctno in (select acctno from #tmp)
	delete from dbo.SPASummary where acctno in (select acctno from #tmp)
	--delete from dbo.agr_temp where acctno in (select acctno from #tmp)
...
GO
/****** Object:  StoredProcedure [dbo].[Step3_TruncateCLTable]    Script Date: 8/6/2020 5:08:32 AM ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
...
END
GO
/****** Object:  StoredProcedure [dbo].[Step4_DenyCLPermission]    Script Date: 8/6/2020 5:08:32 AM ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
